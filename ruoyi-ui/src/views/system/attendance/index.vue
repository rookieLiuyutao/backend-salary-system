<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="attendanceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="缺勤记录id" align="center" prop="absenceId" />
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.absenceId)
    ...
    handleUpdate(row) {
      this.reset();
      const absenceId = row.absenceId || this.ids
      getAttendance(absenceId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const absenceIds = row.absenceId || this.ids;
      this.$modal.confirm('是否确认删除出勤编号为"' + absenceIds + '"的数据项？').then(function() {
        return delAttendance(absenceIds);
      }).then(() => {
        this.getList();
